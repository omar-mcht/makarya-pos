    ...
    $(document).ready(function() {
      ...
      $(document).on('click', '#tambah', function (){
        ...
        var html = "<tr id='baris' " +baris+ ">"
            html+= "<td>"+baris+"</td>"
            html+= "<td contenteditable='true' class='product'><select id='product'  name='product_id' class='form-control select2bs4' style='width: 100%;'> @foreach ($products as $product) <option :selected='transaction_detail.product_id == {{ $product->id }}' value='{{ $product->id }}'>{{ $product->name }}</option> @endforeach`</td>"
